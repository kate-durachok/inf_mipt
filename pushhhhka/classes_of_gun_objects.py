...
class Target:
    def __init__(self, type, radius, window_size_x, window_size_y, height_above_the_gun, radius_of_gyration_min,
        ...
        if (type == 3):
            ...
            self.center_x = randint(0 + radius_of_gyration_max + radius * 2,
                                    window_size_x - radius_of_gyration_max - radius * 2)
            self.center_y = randint(0 + radius_of_gyration_max + radius * 2,
                                    window_size_y - radius_of_gyration_max - radius * 2 - height_above_the_gun)
    ...
    def hit(self, ball_x, ball_y, ball_rad):
        if (ball_rad + self.radius > np.sqrt((self.x_pos - ball_x) ** 2 + (self.y_pos - ball_y) ** 2)):
            if (self.type == 1) or (self.type == 2):
                ...
                self.y_pos = randint(0 + self.radius * 2,
                                     self.window_size_y - self.radius * 2 - self.height_above_the_gun)
            if (self.type == 2):
            ...
            if (self.type == 3):
                ...
                self.center_x = randint(0 + self.radius_of_gyration_max + self.radius * 2,
                                        self.window_size_x - self.radius_of_gyration_max - self.radius * 2)
                self.center_y = randint(0 + self.radius_of_gyration_max + self.radius * 2,
                                        self.window_size_y - self.radius_of_gyration_max - self.radius * 2 - self.height_above_the_gun)
                self.angle_of_gyration = randint(0, int(pi * 10))
                self.x_pos = self.center_x + int(self.radius_of_gyration * np.cos(self.angle_of_gyration / 10))
                self.y_pos = self.center_y + int(self.radius_of_gyration * np.sin(self.angle_of_gyration / 10))
            return True
