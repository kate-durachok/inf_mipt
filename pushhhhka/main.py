...
target_1 = Target(1, target_radius, window_size_x, window_size_y, height_above_the_gun, radius_of_gyration_min,
                  radius_of_gyration_max, max_speed, min_speed)
target_2 = Target(2, target_radius, window_size_x, window_size_y, height_above_the_gun, radius_of_gyration_min,
                  radius_of_gyration_max, max_speed, min_speed)
target_3 = Target(3, target_radius, window_size_x, window_size_y, height_above_the_gun, radius_of_gyration_min,
                  radius_of_gyration_max, max_speed, min_speed)
...
finished = False
paintbrush = Paintbrush(screen, window_size_y, gun_size_height, gun_size_length, target_radius, RED, BLACK, WHITE,
                        shell_radius, BLACK)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_v and power_up_1:
                ...
                new_ball(x + gun_size_length / 2, power, window_size_y - gun_size_height * 3 / 2, angle, limit_of_speed,
                         limit_of_power, acceleration, 1, balls, number_of_balls, shell_radius)
            if event.key == pygame.K_p and power_up_2:
                power_up_2 = False
                if number_of_guns:
                    x, angle, power = gun_2.shot()
                    new_ball(x + gun_size_length / 2, power, window_size_y - gun_size_height * 3 / 2, angle,
                             limit_of_speed, limit_of_power, acceleration, 2, balls, number_of_balls, shell_radius)
